...
const {F,O,S,N, isObj,needObj} = core.types;

const jqp = require('@lumjs/jquery-plugins').new({setDef: true, load: true});
const $  = jqp.$;
jqp.enable('json-elements', 'exists');

const WebService = require('@lumjs/webservice');
const WebDebug = require('@lumjs/web-debug');
const InitGroup = require('./initgroup');

/**
 * A base class for data models from Lum.js v4. 
 */
class Model
{
  /**
   ...
   *
   *  'observable' {object}   If set, options for the observable() call.
   *  'debug'      {object}   If set, it's default debugging options.
   *
   */
  constructor (conf={})
  {
    console.debug("ModelAPI is deprecated, try @lumjs/web-app instead");

    needObj(conf, "ModelAPI constructor must be passed an object");
    ...
    { // Set up the debugging.
       ...
       * Our debug instance.
       */
      this.debug = new WebDebug(dbgopts);
  ...
  {
    if (extension.prototype instanceof Extension)
    {
  ...
  _load_json_model (name, source)
  {
    ...
    if (element.exists())
    {
      ...
      def(jsondata, 'save', function (target)
      {
        if (!target)
          target = elname;
        $(target).JSON(this);
      ...
      def(jsondata, 'json', function (format)
      {
        const space = (typeof format === N) ? format : undefined;
        const json = JSON.stringify(this, undefined, space);
        if (format === true)
        {
          const fmtJSON = require('@lumjs/formatting/json');
          return fmtJSON(json);
        }
        else
        {
  ...
  static addWS(name, baseUrl, methods, wsOpts={})
  {
    ...
    needObj(wsOpts, "The 'wsOpts' parameter must be an object");
    ...
    { // No methods off the bat? That's okay, we'll use a builder.
      ...
      return new WebService.Builder(wsOpts);
    }
  ...
  static extendWS(wsName, extName, methods, addOpts)
  {
    ...
    { // Let's do this.
      ...
      if (typeof addOpts === O)
      {
        ...
        if (!isObj(methods) && isObj(addOpts.methods))
    ...
    if (!isObj(methods) && !isObj(addOpts))
    { // Let's use a builder like we do in `addWS()`
      addOpts = {};
      return new WebService.Builder(addOpts);
    }
  ...
  static makeAPI ()
  {
    console.debug("makeAPI has been deprecated for a long time");
...
Model.prototype.pre_init = {};
Model.prototype.init = {};
Model.prototype.post_init = {};
 ...
 */
Model.prototype._default_observable = {addme: 'make_observable'};
