...
const {F,S,O,def} = core.types;
const LumPromise = require('@lumjs/compat/v4/promise');
 ...
 * Therefore it's been deprecated for quite some time. New code should use
 * better options, and old code should eventually be updated to get away from
 * this bit of hackery.
 * 
 * @exports module:@lumjs/compat-modelapi/ws-data
 * 
 * @param {function} [Model] - The ModelAPI or sub-class constructor to extend
 * 
 * If not specified, the default `model` module will be used.
 * 
...
module.exports = function(Model)
{
  ...
  {
    var promise = new LumPromise(this.__ws.deferred);
   ...
   *                  The promise is either a jqXHR, or a LumPromise
  ...
  map.list = function (reload, listopts)
  {
    ...
    {
      var promise = new LumPromise(self.__ws.deferred);
  ...
  function add_watch_list (doc, name, subdoc)
  {
    ...
    core.def(doc, name, function (props)
    {
  ...
  map.extendObject = function (doc)
  {
    ...
    core.observable(doc, this._extend_observable);
    ...
    def(doc, 'set', function (prop, value)
    {
      if (typeof prop === S && value !== undefined)
      {
        var meth = 'set_'+prop;
      ...
      else if (typeof prop === O && value === undefined)
      {
  ...
  map.save = function (doc)
  {
    ...
    { // We're making changes to an existing document.
      ...
      { // Find changes.
        ...
        {
          var promise = new LumPromise(true);
  ...
  //      console.log("patch", savedoc);
    ...
    { // We're saving a new document.
      ...
      var savedoc = (typeof doc.serialize === F)
                  ? doc.serialize()
                  : doc;
